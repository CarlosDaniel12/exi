...
# Mapa de cores para destacar cada produto (opcional)
MAPA_CORES = {
    "50277E_5": "#faeba9",
    "10170578202": "#ffffff",
    "68600632": "#aad3df",
    # … cores para mais SKUs …
}

# Caminho onde estão as logos (caso use imagens de marca)
CAMINHO_LOGOS = (
    "C:/meu_app_streamlit/logos"
    if os.path.exists("C:/meu_app_streamlit/logos")
    else "meu_app_streamlit/logos"
)
...
# --- 1) Inicialização de session_state ---
for var, default in [
    ("contagem", {}),
    ("pedidos_bipados", []),
    ("input_codigo", ""),
...
# --- 2) Configurações gerais do layout ---
st.set_page_config(layout="wide")
st.title("Bipe de Produtos")

# --- 3) Uploader de planilhas ---
...
# --- 4) Campo de input para código ou pedido ---
st.text_input(
    "Bipe o código interno, de barras ou número do pedido:",
    key="input_codigo",
)

# --- 5) Processamento dos códigos inseridos ---
codigos = (
    re.split(r"[\s,]+", st.session_state.input_codigo.strip())
    if st.session_state.input_codigo
    else []
)
...
for uploaded_file in st.session_state.uploaded_files:
    ...
    if "sku" not in df.columns or "número pedido" not in df.columns:
        st.error(f"CSV {uploaded_file.name} inválido. Faltam colunas obrigatórias.")
        continue
    ...
    for codigo in codigos:
        ...
        if not pedidos.empty:
            ...
            for sku in pedidos["sku"]:
                st.session_state.contagem[sku] = (
                    st.session_state.contagem.get(sku, 0) + 1
                )
                if sku not in st.session_state.ativos:
                    st.session_state.ativos.append(sku)
        else:
            entrada = f"Pedido {codigo} → SKU não encontrado"
            if entrada not in st.session_state.nao_encontrados:
                st.session_state.nao_encontrados.append(entrada)

# --- 6) Agrupa SKUs por marca para exibição ---
agrupado_por_marca = {}
for sku in st.session_state.ativos:
...
with col1:
    st.button(
        ...
        key="restaurar_todos",
        on_click=lambda: (
            st.session_state.ativos.clear()
            or st.session_state.ativos.extend(
                [item["sku"] for sub in agrupado_por_marca.values() for item in sub]
            )
        ),
    )
with col2:
    if st.button("🔄 Limpar pedidos bipados", key="limpar_pedidos_bipados"):
        st.session_state.pedidos_bipados.clear()
        st.session_state.contagem.clear()
        st.session_state.nao_encontrados.clear()

# --- 8) Exibição dos produtos, por marca ---
for marca, itens in agrupado_por_marca.items():
    with st.expander(f"{marca} ({len(itens)})"):
        for item in itens:
            sku = item["sku"]
            nome = lista_produtos.get(sku, {}).get("nome", sku)
            cor = MAPA_CORES.get(sku, "#ddd")
            cols = st.columns([1, 8, 1])
            cols[1].markdown(
                f"<span style='color:{cor}'>{nome}</span>",
...
# --- 9) Página de resultados / QR Code ---
if st.session_state.pedidos_bipados:
    base_url = "https://seu-app-no-github.io/exi"
    params = {"pedidos": ",".join(st.session_state.pedidos_bipados)}
    ...
    st.markdown(
        f"[Clique aqui para acessar a página de resultados]({full_url})",
        unsafe_allow_html=True,
    )
else:
