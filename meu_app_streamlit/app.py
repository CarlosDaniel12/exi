...
produto_color_mapping = {
    # TSUBAKI, ICE, SENKA, CAROL, BEDHEAD, KERASYS...
    # ... (mantém seu mapeamento completo aqui) ...
}
...
if "resultado" in params:
    ...
    # Agrupa os pedidos por marca a partir de st.session_state.contagem
    agrupado_por_marca = {}
    for codigo, quantidade in st.session_state.get("contagem", {}).items():
    ...
    # Cabeçalho e botão de restaurar todos
    ...
    # Definição de grupos de marcas/corredores
    grupos = [
...
def processar():
    ...
    for uploaded_file in st.session_state.uploaded_files:
        ...
        if "sku" not in df.columns or "número pedido" not in df.columns:
            st.error(f"CSV {uploaded_file.name} inválido. Colunas obrigatórias não encontradas.")
            return
        ...
        for codigo in codigos:
            ...
            if not pedidos.empty:
                for sku in pedidos["sku"]:
                    for sku_individual in str(sku).split("+"):
                        ...
                        if sku_individual in produtos_cadastrados:
                            ...
                            msg = f"Pedido {codigo} → SKU: {sku_individual}"
                            if msg not in st.session_state.nao_encontrados:
                                st.session_state.nao_encontrados.append(msg)
            else:
                # código direto
                if codigo in produtos_cadastrados:
                    ...
                    msg = f"Código direto → SKU: {codigo}"
                    if msg not in st.session_state.nao_encontrados:
                        st.session_state.nao_encontrados.append(msg)

    st.session_state.input_codigo = ""

if st.button("🔄 Limpar pedidos bipados", key="clear_all"):
    st.session_state.contagem.clear()
    st.session_state.nao_encontrados.clear()

# Exibição da bipagem e geração do QR Code
titulo_html = (
    "<div style='text-align:center;'>"
    "<h2>BIPAGEM DE PRODUTOS</h2>"
    "</div>"
)
...
if st.session_state.nao_encontrados:
    ...
    st.markdown(
        f"<div style='background-color:#ffcccc; padding:10px; border-radius:5px; color:red;'>"
        f"⚠️ ATENÇÃO: {qtd_nao} pedido(s) não foram lidos!"
        f"</div>",
        unsafe_allow_html=True
    )
    with st.expander(f"Clique aqui para visualizar os {qtd_nao} pedidos não lidos."):
        for entrada in st.session_state.nao_encontrados:
            st.write(f"- {entrada}")

# Exibe as marcas e produtos bipados
marcas_com_produtos = []
...
for i in range(linhas):
    cols = st.columns(marcas_por_linha)
    for j, marca in enumerate(marcas_com_produtos[i*marcas_por_linha:(i+1)*marcas_por_linha]):
        with cols[j]:
            try:
            ...
            for cod, qtd in st.session_state.contagem.items():
                ...
                if produto and produto["marca"] == marca:
                    st.markdown(f"**{produto['nome']}** | Quantidade: {qtd}")

# Geração do QR Code
to_encode = {"resultado": "1"}
for sku, qtd in st.session_state.contagem.items():
    to_encode[sku] = str(qtd)
query = urllib.parse.urlencode(to_encode)
full_url = f"{st.runtime.scriptrunner._main_dg._get_url()}?{query}"
...
st.image(buf.getvalue(), caption="QR Code para a Página de Resultados")
