...
# If the URL contains the 'resultado' parameter, display the results page.
params = st.query_params  # <-- Notice: no parentheses here!
if "resultado" in params:
    st.title("Resultados dos Produtos Bipados")
    st.markdown("---")
    # Loop through parameters (excluding 'resultado') and display product data:
    for codigo, valores in params.items():
        if codigo == "resultado":
            continue
        quantidade = valores[0]  # Assumes a single quantity per SKU
...
# Function to process input codes
def processar():
    ...
    if uploaded_files:
        for uploaded_file in uploaded_files:
            ...
            # Check if the 'SKU' column exists
...
# If any codes were not found, display them
...
# Display the bipado products grouped by brand
marcas_com_produtos = []
...
# Generate the QR Code that redirects to the results page
if st.session_state.contagem:
    # Adjust base_url for your hosted app's URL
